...
switch (enem_id) {
    case EN.RYKEN:
    	switch(art_event) {
            ...
            //NPC Varaibles
            char_name = "Rykenburn, The Smokescreen Powerhouse";
            ...
            ai_hit_absorption = 0;
            ai_hit_percent = 0;
            percent = 0; //Assume early percent values?
			...
			percent_adj = 0.5; //How much damage gets added to percent for knockback scaling purposes
    		...
    		case EN_EVENT.UPDATE:
                ...
                if team == 0 {
	    			...
	    			if (player_controller == 0 && hitstun <= 0) {
	                    ...
	                    //Set After Team Change - see user_event1
	              //      show_healthbar = true;
            			// is_boss = true;
	         ...
	         //           	if team == 1 {
	                    ...
	                    //If there's a hitbox near that's not gonna hit
	                    //KEEEL
	                    ...
	                    if (ai_attack_timer > 0) {
	                    	...
	                    	if (art_state != PS_ATTACK_GROUND && art_state != PS_ATTACK_AIR) {
	                            ...
	                            if (rel_y <= -96) jump_down = true;
		                    		...
		                    		if (t_xd <= 96) {
			                    		if (t_yd >= 64) {
			                    			if decision_random2 <= 70 {
			                    				if is_free next_attack = AT_UAIR;
			                    				else next_attack = AT_UTILT;
			                    		...
			                    		// else if (t_xd <= 32) next_attack = AT_NSPECIAL;
			                    			...
			                    			else if decision_random2 <= 30 next_attack = AT_JAB;
			                    			else if decision_random2 <= 60 next_attack = AT_FTILT;
			                    			else if decision_random2 <= 100 next_attack = AT_UTILT;
			                    		}
		                    		}
		                    	}
		                    }
	                    ...
	                    //Default attack behavior
		                        ...
		                        if (decision_random == 0 && !committed) {
		                        	...
		                        	if (t_xd <= 86 && t_yd <= 64) {
		                        		if is_free {
			                        		...
			                        		else if decision_random2 <= 40 next_attack = AT_NAIR;
			                        		else if decision_random2 <= 60 next_attack = AT_UAIR;
			                        		else if decision_random2 <= 80 next_attack = AT_FAIR;
		                        		} else if decision_random2 <= 20 next_attack = AT_NSPECIAL;
	                       ...
	                       if (t_xd <= 64 && t_yd <= 64) ai_attack_cooldown = 0; //IF THEY'RE CLOSE ATTACK!!
	                       else ai_attack_cooldown -= 1;
	                    }
	                } else { //If in hitstun
	                	if (art_state != PS_ATTACK_AIR && art_state != PS_ATTACK_GROUND) && hit_count >= hit_count_max*1.5 { //Cheese protection :)
	                    	next_attack = AT_DSPECIAL;
	                    	hit_count = 0;
	                    	// hit_count_max = hit_count_max_avg + random_func(id % 200,hit_count_max_range,true);
	                    	// ai_attack_cooldown = 20;
	                ...
	                //Recovery - gonna just clamp stuff :^)
	                x = clamp(x,stage_l,stage_r);
	                //horizontal recovery
	                // if (x < stage_l || x > stage_r) { //Let him combo offstage :P
	                // 	right_down = (x < stage_l);
	                // 	left_down = (x > stage_r);
	                // 	// if (state != PS_ATTACK_AIR && state != PS_ATTACK_GROUND) && y < stage_y next_attack = AT_FSPECIAL;
	                // }
	                // if (y > stage_y && vsp > 0)
	                // {
	                // 	if (able_to_djump && djumps > 0) jump_down = true;
	                // 	else { //If well bellow the stage
       ...
       //         }
    		...
    		case EN_EVENT.DEATH:
                ...
                if state_timer == 240+45 {
                    sound_play(sound_get("sfx_enemy_hit"));
                    spawn_hit_fx(round(x), round(y - char_height/2), fx_enemy_abyssdeath2);
                    with action_manager array_push(action_queue, [room_id, 0, 14, id]); //Action to spawn the note
                    destroyed = true;
                }
    		break;
    			 ...
    			 with (obj_stage_main) {
                    switch (other.attack) {
                    	...
                    	case AT_DTILT:
                    		...
                    		break;
                    	case AT_FTILT:
                    		set_attack_value(AT_FTILT, AG_SPRITE, sprite_get("enemy_100_ftilt"));
							set_attack_value(AT_FTILT, AG_HURTBOX_SPRITE, sprite_get("enemy_100_ftilt_hurt"));
							...
							set_hitbox_value(AT_FTILT, 1, HG_HIT_LOCKOUT, 15);
							...
							set_hitbox_value(AT_FTILT, 2, HG_HIT_LOCKOUT, 15);
                    	...
                    	case AT_DSPECIAL:
                            ...
                            set_hitbox_value(AT_DSPECIAL, 1, HG_WIDTH, 240);
                            set_hitbox_value(AT_DSPECIAL, 1, HG_HEIGHT, 240);
                            ...
                            set_hitbox_value(AT_DSPECIAL, 2, HG_WIDTH, 360);
                            set_hitbox_value(AT_DSPECIAL, 2, HG_HEIGHT, 360);
                    	...
                    	case AT_UAIR:
							...
							set_hitbox_value(AT_UAIR, 1, HG_WIDTH, 64 + (32));
							set_hitbox_value(AT_UAIR, 1, HG_HEIGHT, 64 + (32));
...
var smoke = instance_create(floor(_x), floor(_y), "obj_stage_article",18); //Ryken's Smoke
