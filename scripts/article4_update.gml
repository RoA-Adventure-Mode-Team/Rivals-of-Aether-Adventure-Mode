...
switch trigger_shape {
    ...
    default:
        ...
        //     with instance_place_list(x,y,obj_stage_article,) if num == 6 other.collis_obj = id;
        // }
        break;
}

if collis_obj != noone && (req_item_id == 0 || (collis_obj.item_id != noone && req_item_id == collis_obj.item_id.item_id)) && //!(req_item_id == 0 || (("num" in collis_obj && collis_obj.num == 10 && collis_obj.item_id == req_item_id))) ||
 (!hold_up || collis_obj.up_down) && (active_scene == 0 || (active_scene > 0 && cur_scene >= active_scene) || (active_scene < 0 && cur_scene <= abs(active_scene))) {
    if trigger_cooldown == 0 {
        ...
        if req_item_id != 0 with collis_obj { //Remove Item
            // if "num" in self && num == 10 && item_id > -1 && item_id == other.req_item_id {
            //     state = 4; //if item, destroy
            // }
            if  item_id > -1 && item_id.item_id != noone && item_id.item_id == other.req_item_id {
                item_id.state = 4; //if player, check for item and destroy
                // item_id = noone;
            }
        }
